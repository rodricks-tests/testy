...
It's also a good idea to go over all of the  in v4 and get a good understanding of what's changed, in case there are other things you need to update in your project that the upgrade tool doesn't catch.
...
Our  will handle most of these changes for you automatically, so we highly recommend using it if you can.
...
We've changed the selector used by the  to address serious performance issues on large pages:
CSS
/* Before */
...
Learn more about registering custom utilities in the .
...
The corePlugins, safelist, and separator options from the JavaScript-based config are not supported in v4.0. To safelist utilities in v4 use .
...
For example, the popular  library for React lets you animate to and from CSS variable values:
JSX
...
To make these definitions available in these contexts, use  to import them without duplicating any CSS in your bundle:
Vue
<template>
<h1>Hello world!</h1>
</template>
