...
The  and  utilities (flex, shrink-0, and p-6) to control the overall layout
The  and  utilities (max-w-sm and mx-auto) to constrain the card width and center it horizontally
The , , and  utilities (bg-white, rounded-xl, and shadow-lg) to style the card's appearance
The  and  utilities (size-12) to set the width and height of the logo image
The  utilities (gap-x-4) to handle the spacing between the logo and the text
The , , and  utilities (text-xl, text-black, font-medium, etc.) to style the card text
...
Designing with constraints — using inline styles, every value is a magic number. With utilities, you’re choosing styles from a , which makes it much easier to build visually consistent UIs.
Hover, focus, and other states — inline styles can’t target states like hover or focus, but Tailwind’s  make it easy to style those states with utility classes.
Media queries — you can’t use media queries in inline styles, but you can use Tailwind’s  to build fully responsive interfaces easily.
...
These prefixes are called  in Tailwind, and they only apply the styles from a utility class when the condition for that variant matches.
...
Tailwind uses this same approach for , , , and more.

### Untitled

Many utilities in Tailwind are driven by , like bg-blue-500, text-xl, and shadow-md, which map to your underlying color palette, type scale, and shadows.
...
This group-* syntax works with other variants too, like group-focus, group-active, and .
For really complex scenarios (especially when styling HTML you don't control), Tailwind supports  which let you write any selector you want, directly in a class name:
HTML
...
When duplication is localized to a group of elements in a single file, the easiest way to deal with it is to use  to quickly select and edit the class list for each element at once:
...
Here's what a btn-primary class might look like, using  to keep the design consistent:
Save changes
HTML
